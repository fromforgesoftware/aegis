...
func usecasesFxModule() fx.Option {
	return fx.Module("aegis:usecases",
		fx.Provide(
			...
			fx.Annotate(app.NewProfileUsecase, fx.As(new(app.ProfileUsecase))),
			fx.Annotate(db.NewPreferenceStore, fx.As(new(app.PreferenceStore))),
...
func transportFxModule() fx.Option {
	return fx.Module("aegis:transport",
		...
		kitrest.NewFxController(aegishttp.NewPreferenceController),
		kitrest.NewFxController(aegishttp.NewProfileController),
